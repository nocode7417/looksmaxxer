...
enum ChewingLevel {
  ...
  /// Alias for dailyTargetMinutes
  int get targetMinutes => dailyTargetMinutes;
  ...
  /// Get level from target minutes
  static ChewingLevel fromTargetMinutes(int minutes) {
    for (final level in ChewingLevel.values) {
      if (level.dailyTargetMinutes == minutes) {
        return level;
      }
    }
    // Default to closest match
    if (minutes <= AppConstants.chewingBeginnerMinutes) {
      return ChewingLevel.beginner;
    } else if (minutes <= AppConstants.chewingIntermediateMinutes) {
      return ChewingLevel.intermediate;
    } else {
      return ChewingLevel.advanced;
    }
  }
}
